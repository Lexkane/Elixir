def module ElixirConcurrency.Queue do
    def new(max_size) do
    ...
    {{:ok,queue},len+1}
    {queue,len,max_size}->
    if len ==max_size do{:buffered,queue}
    else
        {{:ok,:queue.in(item,queue)},len +1,max)size}
    end
    end
    end
...
case bounded_queue do
{{[],[]},_,_}->{:empty,queue}
{queue,len,buffer}->{{:value,item},new_queue}=:queue.out(queue)
:queue.out(queue)
...
{{:value,item},{new_queue,size,buffer_type}}->
    ...
    {:replty,{item,new_queue},{new_queue,len,max}}
    end
    end
